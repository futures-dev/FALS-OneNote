...
OSF.getSupportedLocale = function OSF$getSupportedLocale(
  locale,
  defaultLocale
) {
  ...
  var LoadScriptHelper = (function () {
    ...
    LoadScriptHelper.prototype.logScriptLoading = function (
      ...
      if (OSF.AppTelemetry && OSF.AppTelemetry.onScriptDone) {
        ...
        var scriptTelemetry = new ScriptTelemetry(
          scriptId,
          startTime,
          msResponseTime
        );
    ...
    LoadScriptHelper.prototype.getScriptEntryByUrl = function (url) {
      for (var key in this.loadedScriptByIds) {
        ...
        if (
          this.loadedScriptByIds.hasOwnProperty(key) &&
          scriptEntry.url === url
        ) {
          return scriptEntry;
        }
    ...
    LoadScriptHelper.prototype.loadScriptInternal = function (
      ...
      if (url) {
        ...
        var loadedScriptEntry =
          scriptId && this.loadedScriptByIds[scriptId]
            ? this.loadedScriptByIds[scriptId]
            : this.getScriptEntryByUrl(url);
        if (
          !loadedScriptEntry ||
          loadedScriptEntry.hasError ||
          loadedScriptEntry.url.toLowerCase() != url.toLowerCase()
        ) {
          ...
          var onLoadCallback = function OSF_OUtil_loadScript$onLoadCallback() {
            ...
            var pendingCallbackCount =
              loadedScriptEntry.pendingCallbacks.length;
...
OSF.InitializationHelper = function OSF_InitializationHelper(
  hostInfo,
  webAppState,
  context,
  settings,
...
OSF._OfficeAppFactory = (function OSF__OfficeAppFactory() {
  ...
  if (window.performance && window.performance.now) {
    _loadScriptHelper.logScriptLoading(
  ...
  var getQueryStringValue = function OSF__OfficeAppFactory$getQueryStringValue(
    paramName
  ) {
    var hostInfoValue;
  ...
  var compareVersions = function _compareVersions(version1, version2) {
    ...
    for (iter in splitVersion1) {
      if (parseInt(splitVersion1[iter]) < parseInt(splitVersion2[iter])) {
        ...
        parseInt(splitVersion1[iter]) > parseInt(splitVersion2[iter])
      ) {
        return true;
      }
    }
  ...
  var _retrieveHostInfo = function OSF__OfficeAppFactory$_retrieveHostInfo() {
    ...
    if (hostInfoValue) {
      ...
      _hostInfo.hostType =
        typeof items[0] == "undefined" ? "" : items[0].toLowerCase();
      _hostInfo.hostPlatform =
        typeof items[1] == "undefined" ? "" : items[1].toLowerCase();
      _hostInfo.hostSpecificFileVersion =
        typeof items[2] == "undefined" ? "" : items[2].toLowerCase();
      _hostInfo.hostLocale =
        typeof items[3] == "undefined" ? "" : items[3].toLowerCase();
      _hostInfo.osfControlAppCorrelationId =
        typeof items[4] == "undefined" ? "" : items[4];
      if (_hostInfo.osfControlAppCorrelationId == "telemetry") {
        _hostInfo.osfControlAppCorrelationId = "";
      }
      _hostInfo.isDialog =
        typeof items[5] != "undefined" && items[5] == "isDialog" ? true : false;
      var hostSpecificFileVersionValue = parseFloat(
        _hostInfo.hostSpecificFileVersion
      );
  ...
  var getAppContextAsync = function OSF__OfficeAppFactory$getAppContextAsync(
    wnd,
    gotAppContext
  ) {
  ...
  var initialize = function OSF__OfficeAppFactory$initialize() {
    ...
    var loadLocaleStrings = function OSF__OfficeAppFactory_initialize$loadLocaleStrings(
      appLocale
    ) {
      ...
      var loadLocaleStringCallback = function OSF__OfficeAppFactory_initialize$loadLocaleStringCallback() {
        if (
          ...
          typeof Strings.OfficeOM == "undefined"
        ) {
          if (!fallbackLocaleTried) {
            fallbackLocaleTried = true;
            var fallbackLocaleStringFile =
              basePath +
      ...
      var localeStringFile =
        basePath +
        OSF.getSupportedLocale(appLocale, OSF.ConstantNames.DefaultLocale) +
        "/" +
        OSF.ConstantNames.OfficeStringJS;
      _loadScriptHelper.loadScript(
        localeStringFile,
    ...
    var onAppCodeAndMSAjaxReady = function OSF__OfficeAppFactory_initialize$onAppCodeAndMSAjaxReady(
      loadSuccess
    ) {
      if (loadSuccess) {
        _initializationHelper = new OSF.InitializationHelper(
          _hostInfo,
          _WebAppState,
          _context,
          _settings,
        ...
        if (
          _hostInfo.hostPlatform == "web" &&
          _initializationHelper.saveAndSetDialogInfo
        ) {
          _initializationHelper.saveAndSetDialogInfo(
            getQueryStringValue("_host_Info")
          );
        }
        ...
        getAppContextAsync(_WebAppState.wnd, function (appContext) {
            ...
            if (appContext.get_appMinorVersion) {
              ...
              if (!isIOS) {
                if (isNaN(appContext.get_appMinorVersion())) {
                  ...
                  hostVersionItems.length > 1 &&
                  !isNaN(Number(hostVersionItems[1]))
                ) {
                  appContext._appMinorVersion = parseInt(hostVersionItems[1]);
                }
              }
                ...
                if (initializedDeclared) {
                  if (_initializationHelper.prepareApiSurface) {
                    Microsoft.Office.WebExtension.initialize(
                      _initializationHelper.getInitializationReason(appContext)
                    );
                  } else {
                    _initializationHelper.prepareRightBeforeWebExtensionInitialize(
                      appContext
                    );
                  }
          ...
          if (
            !_loadScriptHelper.isScriptLoading(
              OSF.ConstantNames.OfficeStringsId
            )
          ) {
          ...
          _loadScriptHelper.waitForScripts(
            ...
            function () {
              ...
              _initializationHelper.loadAppSpecificScriptAndCreateOM(
                appContext,
                appReady,
                basePath
              );
    ...
    var onAppCodeReady = function OSF__OfficeAppFactory_initialize$onAppCodeReady() {
      ...
      if (
        _loadScriptHelper.isScriptLoading(OSF.ConstantNames.MicrosoftAjaxId)
      ) {
        _loadScriptHelper.waitForScripts(
          [OSF.ConstantNames.MicrosoftAjaxId],
          onAppCodeAndMSAjaxReady
        );
      } else {
        _loadScriptHelper.waitForFunction(
          isMicrosftAjaxLoaded,
          onAppCodeAndMSAjaxReady,
          500,
          100
        );
    ...
    if (_hostInfo.isO15) {
      _loadScriptHelper.loadScript(
        basePath + OSF.ConstantNames.O15InitHelper,
      ...
      hostSpecificFileName =
        ...
        ".debug.js";
      _loadScriptHelper.loadScript(
        basePath + hostSpecificFileName.toLowerCase(),
        OSF.ConstantNames.HostFileId,
        onAppCodeReady
      );
    }
    ...
    if (requiresMsAjax && !isMicrosftAjaxLoaded()) {
      ...
      _loadScriptHelper.loadScriptParallel(
        msAjaxCDNPath,
        OSF.ConstantNames.MicrosoftAjaxId
      );
    }
    window.confirm = function OSF__OfficeAppFactory_initialize$confirm(
      message
    ) {
    ...
    window.prompt = function OSF__OfficeAppFactory_initialize$prompt(
      message,
      defaultvalue
    ) {
    ...
    var isOutlookAndroid =
      _hostInfo.hostType == "outlook" && _hostInfo.hostPlatform == "android";
    if (!isOutlookAndroid) {
    ...
    getCachedSessionSettingsKey: function OSF__OfficeAppFactory$getCachedSessionSettingsKey() {
      return (
        (_WebAppState.conversationID != null
          ? _WebAppState.conversationID
          : _appInstanceId) + "CachedSessionSettings"
      );
    },
